...
* The following pages are currently updatable by editing the relevant CSVs:
    + ### [Home](https://rajlabmssm.github.io/RajLab_website/index.html): 
        ...
        - `Lab_Activities.csv` = Add images from various lab events (e.g. conferences, socials) by adding the "Event" name, "Year", and the path to the "Image" (to each respective column). Each row is a new image. The script will automatically organize the images by event and generate button links above.
